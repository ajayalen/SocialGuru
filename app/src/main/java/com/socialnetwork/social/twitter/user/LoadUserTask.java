...
public class LoadUserTask extends AsyncTask<OAuth, Void, User>
{
	...
	protected User doInBackground(OAuth... params)
	{
		...
		try
		{
			Twitter ins = TwitterHelper.getInstance().getTwitterAuthed(token.getOauthToken(),token.getOauthTokenSecret());
	...
	protected void onPostExecute(User user)
	{
		...
		if (flickrjAndroidSampleActivity instanceof LinkedSocialProfileActivity)
			((LinkedSocialProfileActivity) flickrjAndroidSampleActivity).setUserTwitter(user);
